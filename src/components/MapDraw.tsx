...
async function findNearestStreetName(points: Point[]): Promise<string | null> {
  ...
  // Tentar o ponto médio primeiro
  const midIndex = Math.floor(points.length / 2);
  let streetName = await getStreetName(points[midIndex]);
  
  if (streetName) return streetName;

  // Se não encontrou, tentar alguns pontos ao longo da linha
  const samplePoints = [
    points[0],
...
export default function MapDraw() {
  ...
  const handleSaveRua = async () => {
    ...
    // Calcular comprimento e perímetro de roçada
    const totalDistance = calculateTotalDistance();
    const perimetro = totalDistance * 2;
    
    // Preencher formulário com valores iniciais
    const currentDateTime = getCurrentDateTime();
    setNomeDaRua(name || '');
    setDataCadastro(currentDateTime);
    setComprimento(totalDistance.toFixed(2));
    setPerimetroRocada(perimetro.toFixed(2));
    setNotasSobreRua('');
            ...
            <nav className="flex items-center gap-2 sm:gap-3 ml-auto">
              ...
              <div className="hidden lg:flex items-center gap-3">
                ...
                <button
                  className={`px-5 py-2.5 border-2 font-medium rounded-lg bg-transparent active:scale-95 transition-all duration-200 ease-out ${
                    points.length < 2
                      ? 'border-blue-700 text-blue-600 cursor-not-allowed opacity-50'
                      : 'border-blue-500 text-blue-400 hover:border-blue-400 hover:text-blue-300 hover:shadow-lg hover:shadow-blue-500/50'
                  }`}
                  onClick={handleSaveRua}
        ...
        <div className="absolute inset-0 z-[2000] flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-2xl p-8 max-w-md w-full mx-4">
            ...
            <p className="text-lg text-slate-700 mb-6">
              <span className="font-medium">{savedStreetName || 'Rua'}</span> foi salva com sucesso!
            </p>
            <button
